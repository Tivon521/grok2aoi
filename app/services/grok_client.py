...
from app.services.token_manager_adapter import token_manager
...
class GrokClient:
    ...
    def _get_api_endpoint():
        """动态获取 API endpoint"""
        # 使用 Tomiya233 的默认值
        return "https://grok.com/rest/app-chat"
    
    @staticmethod
    def _get_new_conversation_url():
        return f"{GrokClient._get_api_endpoint()}/conversations/new"
    
    @staticmethod
    def _get_continue_conversation_url(conversation_id):
        return f"{GrokClient._get_api_endpoint()}/conversations/{conversation_id}/responses"
    
    @staticmethod
    def _get_share_conversation_url(conversation_id):
        return f"{GrokClient._get_api_endpoint()}/conversations/{conversation_id}/share"
    
    @staticmethod
    def _get_clone_share_link_url(share_link_id):
        return f"{GrokClient._get_api_endpoint()}/share_links/{share_link_id}/clone"
    ...
    async def chat(
        ...
        **kwargs,
        ...
        """
        # 确保 conversation_manager 已初始化
        if not conversation_manager.initialized:
    ...
    async def _do_chat_request(
        ...
        # 思考过程：由全局配置控制是否展示
        show_thinking = get_config("chat.show_thinking", True)
        ...
        if context:
            ...
            url = GrokClient._get_continue_conversation_url(context.conversation_id)
            ...
            url = GrokClient._get_new_conversation_url()
        ...
        proxies = (
            {"http": get_config("proxy.url", ""), "https": get_config("proxy.url", "")}
            if get_config("proxy.url", "")
            else None
        )
        ...
        response = await session.post(
            ...
            timeout=get_config("chat.stream_timeout", 300),
            stream=True,
            proxies=proxies,
        )
        
        # 调试日志
        logger.info(f"[GrokClient] 请求 URL: {url}")
        logger.info(f"[GrokClient] Cookie: {headers.get('Cookie', '')[:60]}...")
        logger.info(f"[GrokClient] Payload keys: {list(payload.keys())}")
        logger.info(f"[GrokClient] Response status: {response.status_code}")
    ...
    async def _share_conversation(
        ...
        url = GrokClient._get_share_conversation_url(conversation_id)
    ...
    async def _clone_conversation(
        ...
        url = GrokClient._get_clone_share_link_url(share_link_id)
                                ...
                                if not is_image_mode:
                                    ...
                                    if message_tag == "tool_usage_card":
                                        if (
                                            token_text
                                            and show_thinking
                                            and get_config("chat.show_search", True)
                                        ):
                                    ...
                                    if message_tag == "raw_function_result":
                                        if show_thinking and get_config("chat.show_search", True):
                                            rollout_id = response_data.get(
                                                "rolloutId", ""
                                            )
                                            prefix = (
    ...
    async def _append_images(content: str, images: list, auth_token: str) -> str:
        ...
        base_url = get_config("app.base_url", "").rstrip("/") if get_config("app.base_url", "") else ""
